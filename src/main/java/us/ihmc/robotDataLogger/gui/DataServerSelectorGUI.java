...
/**
 * Simple popup GUI to select which host to connect to. Has the ability for the user to specify the
 * host manually. Manually added IP's get stored for later use in ~/.ihmc/ControllerHosts.yaml
 *
 * @author Jesper Smith
 */
public class DataServerSelectorGUI implements HostAddedListener
{
   ...
   public void hostAdded(String host, String port)
   {
      try
      {
         ...
         try
         {
            StaticHostListLoader.save(client.getPersistantHostList());
         }
         catch (IOException e)
         {

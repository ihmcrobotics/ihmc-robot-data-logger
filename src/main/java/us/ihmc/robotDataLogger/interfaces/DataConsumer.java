...
public interface DataConsumer
{
   ...
   /**
    * Send a command to the server. If the command is a broadcast command, this gets broadcast to all
    * clients (including this one) by the server. If no session is available, this request gets
    * silently ignored.
    *
    * @throws IOException
    */
   ...
   void startSession(IDLYoVariableHandshakeParser parser, YoVariableClientImplementation yoVariableClient, VariableChangedProducer variableChangedProducer,
                     TimestampListener timeStampListener, CommandListener clearLogListener, DebugRegistry rtpsDebugRegistry)
         throws IOException;
